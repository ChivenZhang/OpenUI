...
static inline uint8_t get_border_radius_bottom_left(const css_computed_style
		*style, css_fixed *length, css_unit *unit)
{
	uint32_t bits = style->i.bits[BORDER_RADIUS_BOTTOM_LEFT_INDEX];
	bits &= BORDER_RADIUS_BOTTOM_LEFT_MASK;
	bits >>= BORDER_RADIUS_BOTTOM_LEFT_SHIFT;
	
	/* 7bits: uuuuutt : unit | type */
	if ((bits & 0x3) == CSS_BORDER_RADIUS_SET) {
		*length = style->i.border_radius_bottom_left;
		*unit = bits >> 2;
	}
...
static inline uint8_t get_border_radius_bottom_right(const css_computed_style
		*style, css_fixed *length, css_unit *unit)
{
	uint32_t bits = style->i.bits[BORDER_RADIUS_BOTTOM_RIGHT_INDEX];
	bits &= BORDER_RADIUS_BOTTOM_RIGHT_MASK;
	bits >>= BORDER_RADIUS_BOTTOM_RIGHT_SHIFT;
	
	/* 7bits: uuuuutt : unit | type */
	if ((bits & 0x3) == CSS_BORDER_RADIUS_SET) {
		*length = style->i.border_radius_bottom_right;
		*unit = bits >> 2;
	}
...
static inline uint8_t get_border_radius_top_left(const css_computed_style
		*style, css_fixed *length, css_unit *unit)
{
	uint32_t bits = style->i.bits[BORDER_RADIUS_TOP_LEFT_INDEX];
	bits &= BORDER_RADIUS_TOP_LEFT_MASK;
	bits >>= BORDER_RADIUS_TOP_LEFT_SHIFT;
	
	/* 7bits: uuuuutt : unit | type */
	if ((bits & 0x3) == CSS_BORDER_RADIUS_SET) {
		*length = style->i.border_radius_top_left;
		*unit = bits >> 2;
	}
...
static inline uint8_t get_border_radius_top_right(const css_computed_style
		*style, css_fixed *length, css_unit *unit)
{
	uint32_t bits = style->i.bits[BORDER_RADIUS_TOP_RIGHT_INDEX];
	bits &= BORDER_RADIUS_TOP_RIGHT_MASK;
	bits >>= BORDER_RADIUS_TOP_RIGHT_SHIFT;
	
	/* 7bits: uuuuutt : unit | type */
	if ((bits & 0x3) == CSS_BORDER_RADIUS_SET) {
		*length = style->i.border_radius_top_right;
		*unit = bits >> 2;
	}
