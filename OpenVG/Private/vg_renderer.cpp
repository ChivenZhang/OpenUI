...
bool VG_Init(VGState* g, int width, int height) {
	...
	/* ── 6g. Initialise gradient UBO with sensible defaults ── */
	memset(&g->ubo, 0, sizeof(g->ubo));
	g->ubo.scale[0] = 1.0f; g->ubo.scale[1] = 1.0f;
	g->ubo.count = 2;
	g->ubo.extend = 0;  /* PAD */
	/* Black → White */
	g->ubo.colors[0][0] = 0; g->ubo.colors[0][1] = 0;
	g->ubo.colors[0][2] = 0; g->ubo.colors[0][3] = 1;
	g->ubo.stops[0] = 0.0f;
	g->ubo.colors[1][0] = 1; g->ubo.colors[1][1] = 1;
	g->ubo.colors[1][2] = 1; g->ubo.colors[1][3] = 1;
	g->ubo.stops[1] = 1.0f;
