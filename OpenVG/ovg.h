...
enum class vg_pattern_type_t :uint8_t {
	...
	VG_PATTERN_TYPE_SURFACE,       /*!< vkvg surface pattern */
...
struct canvas_cb {
	mem_resource_t* ac = 0;
	// 路径操作
	rvg_path_t* (*new_path)(mem_resource_t* ac);
	...
	void(*set_path)(rvg_t* v, rvg_path_t* path, vg_state_save_t* st);
	void(*stroke)(rvg_t* v);
	...
	// 添加文本，风格，渲染区可选
	void (*add_text)(drawlist_t* dc, text_st* p, text_style* ts, text_box_t* box);
	// 普通图片，支持九宫格、混合颜色
	void (*add_image)(drawlist_t* dc, image_dr* r);
	// 原始三角形，输入0则不修改
